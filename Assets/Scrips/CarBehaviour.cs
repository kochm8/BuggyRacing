...
public class CarBehaviour : MonoBehaviour {
    ...
    private ParticleSystem.EmissionModule _finishParticle;
    ...
    void Start () {
        ...
        */
        ...
        _finishParticle = GameObject.Find("FinishPS").GetComponent<ParticleSystem>().emission;
        _finishParticle.enabled = false;
    ...
    void FixedUpdate()
    {
        ...
        if (isFinished)
        {
            wheelFL.brakeTorque = 1;
            wheelFR.brakeTorque = 1;
            wheelRL.brakeTorque = 1;
            wheelRR.brakeTorque = 1;
            wheelFL.motorTorque = 0;
            wheelFR.motorTorque = 0;

            _finishParticle.enabled = true;

            var fishishParticle = GameObject.Find("FinishPS").GetComponent<ParticleSystem>();
            var r = UnityEngine.Random.Range(0.0f, 1.0f);
            var g = UnityEngine.Random.Range(0.0f, 1.0f);
            var b = UnityEngine.Random.Range(0.0f, 1.0f);
            var a = UnityEngine.Random.Range(0.0f, 1.0f);

            Color color = new Color(r, g, b);
            fishishParticle.startColor = color;

            _engineAudioSource.volume = 0.3f;
